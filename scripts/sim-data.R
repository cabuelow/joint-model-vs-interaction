...
fit_lm_categories <- lm(logbio ~ big + old + small + young + restrictions + enviro, dat2)
fit_lm_categories
...
plot_dat %>% 
  ...
  geom_point(aes(x = linear_model, y = coefficient, color = 'Linear model'), size = 3, alpha = 0.5) +
  ...
  geom_point(aes(x = joint_model, y = coefficient, color = 'Joint model')) +
...
ggsave('outputs/plot.png', height = 3, width = 4)
